...
| 狀態 | 視覺樣式 | 使用情境 |
| --- | --- | --- |
| `default` | 品牌色填充/框線 | 一般可操作狀態 |
| `hover` | 略深色背景 | 滑鼠懸停 |
| `disabled` | 灰底灰字，opacity 0.5，cursor not-allowed | 無操作權限、條件未滿足 |
| `loading` | Spinner + 文字「處理中…」，進入 disabled 狀態 | 非同步操作進行中 |
| `read-only` | **不顯示按鈕**，改以純文字或 Badge 呈現對應狀態值 | 僅供閱讀的表單場景（預覽模式、已鎖定欄位）；按鈕本身不設 read-only，若操作不可執行應改用 disabled 或隱藏 |
...
**Selected Row（單選/多選已勾）**

```
┌─────────────────────────────────────────────────────────────┐
│  ☑  │  Episode 1   │  2026-01-01 │  ● 已發佈 │  ⋮        │  ← 選中行
└─────────────────────────────────────────────────────────────┘
     bg: var(--primary-muted)  /* 淺珊瑚粉底 */
     border-left: 3px solid var(--primary)
     checkbox: checked state, brand color fill
```

**Bulk Selection Toolbar（多選浮出工具列）**

當勾選 ≥ 2 筆時，表頭列替換為 Bulk Toolbar：

```
┌─────────────────────────────────────────────────────────────┐
│  ☑  已選擇 3 筆    │  批次發佈  │  批次刪除  │  取消選擇   │
└─────────────────────────────────────────────────────────────┘
     bg: var(--primary-muted)
     height: 48px（與表頭同高）
     actions: Ghost buttons，刪除使用 destructive color
     「取消選擇」在最右，Ghost style
```
...
| 狀態 | 視覺 |
| --- | --- |
| Drag handle | 左側顯示 `GripVertical` icon（16px，gray-400），hover 時才顯示 |
| Dragging | 被拖拽行：`opacity: 0.5`，`bg: white`，帶 `shadow-md`，游標 `grabbing` |
| Drop zone | 目標插入位置：`2px solid var(--primary)` 橫線指示 |
...
| 狀態 | 邊框 | 背景 | 額外視覺 |
| --- | --- | --- | --- |
| `default` | `1px solid var(--border)` gray-300 | white | — |
| `focus` | `2px solid var(--ring)` primary-500 | white | Focus ring offset 2px |
| `error` | `1px solid var(--error-foreground)` 紅色 | `var(--error)` 極淡紅底 | 右側顯示 `AlertCircle` icon（紅色）；欄位下方顯示錯誤訊息 `font-size: 12px, color: --error-foreground` |
| `success` | `1px solid var(--success-foreground)` 綠色 | `var(--success)` 極淡綠底 | 右側顯示 `CheckCircle` icon（綠色） |
| `disabled` | `1px solid var(--border)` gray-200 | `var(--muted)` gray-100 | `opacity: 0.6`，`cursor: not-allowed`，文字 `--muted-foreground` |
| `read-only` | 無邊框 | `var(--muted)` gray-50 | 無 focus ring；`cursor: default`；值以純文字色呈現，不可編輯 |
...
│  │  (已輸入的值)                                  │  │
│  └────────────────────────────────────────────────┘  │
│  ⚠ 節目名稱不能超過 100 個字元                       │  ← 錯誤訊息
└─────────────────────────────────────────────────────┘
    icon: AlertCircle 16px, color: --error-foreground
    text: 12px, color: --error-foreground
    margin-top: 4px
```
...
- 完整互動狀態同 Input（error / success / disabled / read-only 規則相同）
...
> 模組是「元件的組合 pattern」，定義多個元件如何協作形成可複用的頁面區塊。

### 3.1 頁面標頭模組 (Page Header)

**組成元件：**
- Page Title（必要）
- Tabs（可選，使用 2.4 Tab 導航規範）
- Actions 區塊（可選，通常為 Primary / Secondary Button）
...
│  ─────────────                         │  + 上傳  │             │  ← 標題 + Actions 對齊
│                                        └──────────┘             │
│  全部  │  已發佈  │  草稿  │  排程                              │  ← Tabs（可選）
│  ────                                                            │
└─────────────────────────────────────────────────────────────────┘

Page Title: font-size 24px, font-weight 600, color --foreground
標題與 Actions: 垂直對齊 center，flex justify-between
標題與 Tabs 間距: 16px
Tabs 與內容區間距: 24px（由內容區 padding-top 控制）
```
...
| 狀態 | 說明 |
| --- | --- |
| 無 Tabs | Actions 靠右對齊，標題行高度 auto（含 padding） |
| 有 Tabs + 有 Actions | Actions 與 Page Title 同行，Tabs 在下方 |
| 有 Tabs + 無 Actions | 僅顯示 Tabs，Page Title 獨佔一行 |

---

### 3.2 統計卡片群組 (Metric Cards)

**組成元件：**
- MetricCard × N（每張卡片使用 2.2 Card 規範，`default` 變體）
- Grid 容器
...
```
┌─────────────────────────────┐
│  總播放數                    │  ← Label: 14px, --muted-foreground
│                             │
│  128,450                    │  ← 數字: 30px, font-weight 700, --foreground
│                             │
│  ↑ 12.5%  vs 上月           │  ← 趨勢: 12px, 上升綠色/下降紅色 + arrow icon
└─────────────────────────────┘
...
| 方向 | Icon | 顏色 |
| --- | --- | --- |
| 上升 | `TrendingUp` 16px | `--success-foreground` |
| 下降 | `TrendingDown` 16px | `--error-foreground` |
| 持平 | `Minus` 16px | `--muted-foreground` |
...
### 3.3 資料表格模組 (Data Table Module)

**組成元件：**
1. Filter Bar（搜尋 + 篩選）
2. Table（使用 2.3 表格規範，含補充狀態）
3. Pagination
...
┌─────────────────────────────────────────────────────────────────┐
│  ┌──────────────────────────┐  ┌──────────┐  ┌──────────┐      │
│  │  🔍 搜尋單集名稱...       │  │  狀態 ▼  │  │  日期 ▼  │      │  ← Filter controls
│  └──────────────────────────┘  └──────────┘  └──────────┘      │
└─────────────────────────────────────────────────────────────────┘

搜尋框: flex-grow 1，最小寬度 200px
Filter Dropdowns: 寬度 auto（依內容），最小 120px
Filter Bar 與 Table 間距: 16px
Filter Bar 高度: 40px（與 Input sm 一致）
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│  顯示 1–20，共 143 筆               ← 上一頁  1  2  3  下一頁 → │
└─────────────────────────────────────────────────────────────────┘

左側: 12px, --muted-foreground
右側: 頁碼按鈕，active page 使用 Primary 樣式
Pagination 與 Table 間距: 16px
```

---

### 3.4 導覽選單模組 (Navigation)

**組成元件（Sidebar 結構）：**

```
┌──────────────────────┐
│  [Logo]  Firstory    │  ← Logo 區：高度 64px，padding 16px 20px
├──────────────────────┤
│                      │
│  ▣ 管理總覽          │  ← Nav Item（Active）：bg --primary-muted，左邊線 3px primary
│  ♪ 單集管理          │  ← Nav Item（Default）：bg transparent，hover bg --muted
│  👥 受眾分析          │
│  💰 營利管理          │
│  ⚙ 設定              │
│                      │
├──────────────────────┤
│  [Avatar] 用戶名稱   │  ← UserInfo 區：底部固定，height 64px
│           email      │
└──────────────────────┘
...
| 狀態 | 背景 | 文字 | 邊線 |
| --- | --- | --- | --- |
| `default` | transparent | `--muted-foreground` | 無 |
| `hover` | `var(--muted)` | `--foreground` | 無 |
| `active` | `var(--primary-muted)` | `var(--primary)` | `3px solid var(--primary)` 左側 |

**Nav Item 尺寸：**
- 高度：40px
- Padding：`0 16px`
- Icon：20px，`margin-right: 12px`
- 字體：14px，font-weight 500

**收合/展開：**
- 切換動畫：`width transition 300ms --easing-out`
- 收合時 Tooltip 顯示完整名稱（hover）
- 觸發：`< 1024px` 自動收合；也可手動 toggle

**巢狀選單（二層）：**
- 父項目帶 `ChevronDown`/`ChevronRight` icon
- 子項目 padding-left：`52px`（icon 縮排對齊）
- 展開動畫：`max-height transition 200ms`

---

### 3.5 對話確認模組 (Confirmation Dialog)

**組成元件：**
- Modal（使用 2.7 Modal 規範，`sm` 尺寸 480px）
- 危險操作文案規範
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  此操作無法復原。單集將從 Firstory 及所有已發佈的平台           │
│  永久移除。                                                     │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│                         ┌──────────┐  ┌───────────────────┐    │
│                         │  取消    │  │  確認刪除         │    │  ← Destructive button
│                         └──────────┘  └───────────────────┘    │
└─────────────────────────────────────────────────────────────────┘
```
...
| 欄位 | 格式 | 範例 |
| --- | --- | --- |
| 標題 | `刪除「{物件名稱}」？` | 刪除「2026 年度回顧」？ |
| 說明 | 第一句說明後果，第二句說明影響範圍 | 此操作無法復原。單集將從…永久移除。 |
| 確認按鈕 | `確認{動詞}` | 確認刪除 |
| 取消按鈕 | `取消` | — |

**Destructive Button 位置規則：**
- 位置：Footer 右側（最後一個按鈕）
- 樣式：`variant="destructive"`，使用 `--destructive` 背景色
- 取消按鈕：Secondary 樣式，在 Destructive 按鈕左側
- **禁止**將 Destructive 按鈕放在左側或使用 Primary 樣式
...
| 版本 | 日期 | 變更內容 |
| --- | --- | --- |
| 2.1.0 | 2026-02-23 | 新增第三章模組規範（5 個模組）；補充 Button read-only、Table 選取/多選/拖曳、Input 完整互動狀態；原第三～七章重新編號為四～八章 |
...
*— Firstory Studio Design System v2.1.0 —*
