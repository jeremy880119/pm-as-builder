...
**對應 PRD：** prd-v2.0
**總 Story Points：** 56（P0: 32 pts / P1: 24 pts）
...
| **EP-03: Paywall 觸發升級** | | **P1** | **7** | |
| US-07 | 進階功能標示系統 | | 4 | US-01 |
...
**P1（7 個，24 pts）= 轉化率優化 + 挽留優化 + 體驗優化**
- 免費試用、註冊頁方案選擇、進階功能標示、Paywall、勸退 Modal、升級確認頁、待降級狀態
...
#### 技術備註
- 頁面路徑：`/account`（https://studio.firstory.me/account）
...
#### 技術備註
- 頁面路徑：`/account/subscription/plans`（https://studio.firstory.me/account/subscription/plans）
...
#### Use Case
- **As a** 已登入用戶,
- **I want to** 在所有進階功能入口看到一致的視覺標示，未解鎖時顯示上鎖、已解鎖時顯示進階身份,
...
- 元件需支援兩種狀態：locked（未解鎖，點擊觸發 Paywall Modal）與 unlocked（已解鎖進階標示）
- 具體視覺樣式由設計階段（wireframe）決定，本 Story 定義行為需求
- 標示對象由 Registry `min_tier` 推導：功能的 `min_tier > Free` 即為進階功能
- Paywall 觸發功能（4 項）：`advanced_analytics`（Pro）、`download_report`（Pro）、`episode_flink`（Lite）、`remove_dynamic_ads`（Pro）
- 付費用戶頭像不應使用與進階功能相同的標示，避免混淆
- 待工程確認：是否有其他功能入口需要標示？（如 AI 內容萃取的配額限制提示）
...
- Then: 顯示功能名稱、說明、所需方案（Pro 年繳），含升級 CTA 與「查看所有方案」連結

**Scenario: 從 Paywall 直接升級**
- Given: Free 用戶在 Paywall Modal 中
- When: 用戶點擊升級 CTA
- Then: 跳轉 Stripe Checkout（年繳方案），成功後進入元件 E 確認頁

#### 技術備註
- Paywall Modal 只推薦年繳方案，僅顯示 Pro 或 Enterprise（不顯示 Lite、不顯示月繳）
- 推薦邏輯：功能 `min_tier` 為 Lite 或 Pro → 推 Pro 年繳；功能 `min_tier = Enterprise` → 推 Enterprise 年繳
- CTA 文案依試用資格切換：「升級到 {plan}」或「14 天免費試用 {plan}」
- 次要 CTA：「查看所有方案」→ 方案選擇頁（元件 B），用戶可在該頁看到完整方案與計費週期選擇
...
| User Story | `user-story-v2.3-20260213.md` | ✅ 本文件 |
| Wireframe | `wireframe-v4.1-20260213.html` | ✅ 已存在 |
| Prototype | — | ⬜ 尚未產出 |
...
| v2.3 | 2026-02-13 | US-07 從「紅鎖頭系統」擴展為「進階功能標示系統」（含 locked + unlocked 兩狀態，統一取代紅鎖頭與黃星星），points 3→4；US-08 Paywall Modal 限定只推年繳 Pro/Enterprise；US-02/US-03 技術備註補充頁面路徑；總 Points 55→56，P1 23→24 pts | User Story |
