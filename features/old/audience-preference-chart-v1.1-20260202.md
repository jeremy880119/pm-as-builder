...
> 📸 **示意圖：** 節目數據頁載入後，「聽眾偏好」區塊預設顯示首週表現長條圖，15 集由新到舊排列
> ![預設畫面](figma://default-view)
...
> 📸 **示意圖：** 用戶點擊時間區間下拉選單，選擇「首月表現」，圖表與表格同步更新
> ![切換時間區間](figma://time-range-switch)
...
> 📸 **示意圖：** 用戶點擊排序切換按鈕，長條圖重新排列為下載數由高到低
> ![排序切換](figma://sort-toggle)
...
> 📸 **示意圖：** 圖表兩側出現左右箭頭，用戶點擊左箭頭瀏覽更早單集
> ![分頁瀏覽](figma://pagination)
...
> 📸 **示意圖：** 最新單集的長條為半透明紫色，Hover 顯示「數據累積中（發布不到 7 天）」
> ![數據累積中](figma://accumulating-state)
...
> 📸 **示意圖：** 用戶 Hover 長條，Tooltip 顯示單集標題、Apple/RSS 下載數、總計
> ![Hover Tooltip](figma://hover-tooltip)
...
> 📸 **示意圖：** 用戶點擊表格中某一列，跳轉至該單集的詳細分析頁
> ![前往單集頁](figma://navigate-to-episode)
...
> 📸 **示意圖：** 用戶在搜尋框輸入關鍵字，圖表與表格即時篩選顯示符合結果
> ![搜尋篩選](figma://search-filter)
...
> 📸 **示意圖：** 用戶點擊指標切換，圖表數值更新為重複下載數，其他設定不變
> ![指標切換](figma://metric-switch)
